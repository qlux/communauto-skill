...
Rates updated 2026-08-11 from pricing screenshots provided directly by the user (supersedes
the 31 janvier 2025 PDF grid previously used), including FLEX rates. See
../references/tarifs.md for the full source table, caveats, and things this script
deliberately doesn't model (Tarif Longue distance, damage waiver options, membership
fees, etc).
...
PACKAGES = {
    "liberte": Package(
        ...
        hourly=14.25, day1_cap=55.0, day_add_cap=50.0,
        km_included=75, km_tier1_limit=0, km_tier1_rate=0.0, km_tier2_rate=0.31,
        weekend_surcharge_per_hour=0.35, weekend_surcharge_cap=3.50,
    ),
    "liberte_plus": Package(
        key="liberte_plus", label="Liberté Plus",
        hourly=7.05, day1_cap=50.0, day_add_cap=35.50,
        km_included=0, km_tier1_limit=50, km_tier1_rate=0.29, km_tier2_rate=0.26,
        weekend_surcharge_per_hour=0.35, weekend_surcharge_cap=3.50,
    ),
    "economique": Package(
        key="economique", label="Économique",
        hourly=3.80, day1_cap=31.50, day_add_cap=31.50,
        km_included=0, km_tier1_limit=50, km_tier1_rate=0.49, km_tier2_rate=0.36,
        weekend_surcharge_per_hour=0.35, weekend_surcharge_cap=3.50,
    ),
    "economique_plus": Package(
        key="economique_plus", label="Économique Plus",
        hourly=3.30, day1_cap=26.50, day_add_cap=26.50,
        km_included=0, km_tier1_limit=50, km_tier1_rate=0.41, km_tier2_rate=0.33,
        weekend_surcharge_per_hour=0.35, weekend_surcharge_cap=3.50,
    ),
    "economique_extra": Package(
        key="economique_extra", label="Économique Extra",
        hourly=3.00, day1_cap=23.0, day_add_cap=23.0,
        km_included=0, km_tier1_limit=0, km_tier1_rate=0.0, km_tier2_rate=0.33,
        weekend_surcharge_per_hour=0.35, weekend_surcharge_cap=3.50,
    ),
}
...
FLEX_PER_MINUTE = 0.43
FLEX_HOURLY_EQUIV = 14.25
FLEX_DAY_CAP = 50.0
FLEX_KM_INCLUDED = 75
FLEX_KM_RATE = 0.31

TARIF_TRAVAIL_DAILY = 24.0
TARIF_TRAVAIL_KM_INCLUDED = 40
TARIF_TRAVAIL_KM_RATE = 0.36
TARIF_TRAVAIL_MAX_HOURS = 10

VEHICLE_CLASS_SURCHARGE = {
    "standard": 0.0,
    "family": 0.10,   # +10% on trip cost (time AND km), confirmed against a real receipt
    "minivan": 0.15,  # +15% on trip cost (time AND km), per Communauto's published fee list
}
...
def station_time_cost(pkg: Package, hours: float, weekend_days: int = 0) -> dict:
    ...
    # Weekend surcharge (all station packages carry this per the tariff table's
    # footnote — not applied when the winning price ends up being FLEX).
    surcharge = 0.0
...
def flex_time_cost(hours: float) -> dict:
    """FLEX time cost. Billed per minute (FLEX_PER_MINUTE), but each *elapsed
    clock hour* is capped at the FLEX_HOURLY_EQUIV hourly-equivalent (this
    hour-by-hour capping behavior was confirmed against Communauto's own
    worked examples under the old rate table; the underlying mechanic should
    still hold under the updated 2026-08-11 rates, but the specific worked
    numbers are no longer verified against a published example). The running
    total within a 24h block is additionally capped at the daily FLEX cap."""
    if hours <= 0:
        return {"total": 0.0, "days": []}
    days = []
...
def tarif_travail_cost(hours: float, km: float) -> float:
    """Tarif Travail flat rate. Caller is responsible for checking eligibility
    (weekday, <=10 consecutive counted hours) before treating this as a valid
    candidate — this function just does the arithmetic."""
    return round(TARIF_TRAVAIL_DAILY + max(0.0, km - TARIF_TRAVAIL_KM_INCLUDED) * TARIF_TRAVAIL_KM_RATE, 2)
...
def trip_cost(package: str, hours: float, km: float, is_flex: bool,
              weekend_days: int = 0, vehicle_class: str = "standard",
              is_weekday: bool = False) -> dict:
    pkg = resolve_package(package)
    surcharge_rate = VEHICLE_CLASS_SURCHARGE.get(vehicle_class, 0.0)
    ...
    # Tarif Travail check — Économique Extra only, weekday, <=10 consecutive
    # hours. Applies to station trips directly. For FLEX trips it only applies
    # on the station-comparison side (i.e. when the FLEX trip would end up
    # billed at the station-equivalent rate anyway) — it is NEVER compared
    # against the raw FLEX per-minute price, since Tarif Travail is a
    # station-rate benefit, not a FLEX one.
    tarif_travail_eligible = (
        pkg.key == "economique_extra" and is_weekday and hours <= TARIF_TRAVAIL_MAX_HOURS
    )
    if tarif_travail_eligible:
        tt_total = tarif_travail_cost(hours, km)
        result["tarif_travail_price"] = tt_total
        result["tarif_travail_note"] = (
            f"Weekday trip within {TARIF_TRAVAIL_MAX_HOURS}h qualifies for Tarif "
            f"Travail ({TARIF_TRAVAIL_DAILY:.0f}$ flat + km beyond "
            f"{TARIF_TRAVAIL_KM_INCLUDED}) — applied automatically by Communauto if "
            f"cheaper than the station rate. Not available against FLEX per-minute "
            f"pricing, only when the trip would otherwise be billed at the station "
            f"rate."
        )
    elif pkg.key == "economique_extra" and hours <= TARIF_TRAVAIL_MAX_HOURS:
        result["tarif_travail_note"] = (
            "This trip is short enough for Tarif Travail, but it only applies on "
            "weekdays — pass is_weekday=True / --weekday if that's the case."
        )
    elif pkg.key == "economique_extra" and hours > TARIF_TRAVAIL_MAX_HOURS:
        result["tarif_travail_note"] = (
            f"Tarif Travail doesn't apply: trip exceeds the {TARIF_TRAVAIL_MAX_HOURS}h "
            f"consecutive-hours limit (midnight-6am excluded from that count), so the "
            f"whole trip is billed at the normal rate instead."
        )

    if vehicle_class != "standard":
        result["vehicle_class"] = vehicle_class
        result["vehicle_class_surcharge_rate"] = surcharge_rate

    if not is_flex:
        candidates_std = {"station": station_total}
        if "quick_roundtrip_price" in result:
            candidates_std["quick_roundtrip"] = result["quick_roundtrip_price"]
        if tarif_travail_eligible:
            candidates_std["tarif_travail"] = result["tarif_travail_price"]
        winner = min(candidates_std, key=candidates_std.get)
        best = candidates_std[winner]
        if surcharge_rate:
            best = round(best * (1 + surcharge_rate), 2)
    ...
    if tarif_travail_eligible:
        # Tarif Travail only competes against the station-side price, never
        # the raw FLEX price — but since we're picking a global min anyway,
        # and Tarif Travail can only be cheaper than station (or equal),
        # including it directly in the same min() is safe: it will never beat
        # the FLEX price unless it's also beating the station price, which is
        # the only case where Tarif Travail should apply in the first place.
        candidates["tarif_travail"] = result["tarif_travail_price"]

    winner = min(candidates, key=candidates.get)
    final = candidates[winner]
    if surcharge_rate:
        final = round(final * (1 + surcharge_rate), 2)
...
def extend_vs_new(package: str, day1_hours: float, day1_km: float,
    ...
    if pkg.key == "economique_extra" and day2_is_weekday and day2_hours <= TARIF_TRAVAIL_MAX_HOURS:
        ...
        result["tarif_travail_note"] = (
            ...
            f"({TARIF_TRAVAIL_DAILY:.0f}$ + km beyond {TARIF_TRAVAIL_KM_INCLUDED}), applied "
            f"automatically by Communauto if cheaper. Worth checking against both options above."
        )
...
def main():
    ...
    p1.add_argument("--vehicle-class", default="standard",
                     choices=list(VEHICLE_CLASS_SURCHARGE),
                     help="Vehicle category surcharge on time+km (default: standard, no surcharge). "
                          "family = +10%%, minivan = +15%%.")
    p1.add_argument("--weekday", action="store_true",
                     help="Trip falls on a weekday (Mon-Fri) — enables Tarif Travail "
                          "consideration for Économique Extra trips <=10h.")
    ...
    if args.command == "trip-cost":
        out = trip_cost(args.package, args.hours, args.km, args.flex, args.weekend_days,
                         args.vehicle_class, args.weekday)
